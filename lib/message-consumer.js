...
MessageConsumer.prototype.rebalance = function (cb /* (err) */) {
    ...
    async.parallel([
        ...
        var peers = vals[0],
            ...
            newPayloads = self._makeNewPayloads(topics, consumersPerTopicMap, partitionsPerTopicMap); // never self.consumer.payloads.push(newPayload)
        ...
        function potluck (cb) { // attempt to give and take partitions
            async.series([
                ...
                self.thunk('_overridePayloads', intersection), // parallel?
                self.thunk('_givePartitions', give), // parallel?
...
MessageConsumer.prototype._overridePayloads = function (intersection, cb) {
    debug('stopping fetchers');
    this.consumer.payloads = intersection;
    cb();
};

MessageConsumer.prototype._givePartitions = function (give, cb) {
    var self = this;
