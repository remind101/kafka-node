...
MessageConsumer.prototype._prepareConsumer = function () {
    var self = this;
    // accounting
    ...
    // monkey-patch: on fetch, copy payload.busy from maps with O(1) update
    ...
    this.consumer.on('message', function (message) {
        ...
        if (self.lastCommittedOffset[key] == message.offset - 1) { // 1st entry in queue
            self._emitMessage(key, message.offset);
        }
    });
    this.consumer.on('error', function (err) { // TODO handle this
...
MessageConsumer.prototype._emitMessage = function (key, offset) { // with a callback
    var self = this;
    ...
    self.emit('message', message, function () {
        ...
        self.commit(message, function (err) {
            ...
            debug('committing', key, self.lastCommittedOffset[key], '<', offset, '<=', self.lastReceivedOffset[key]);
            if (self.lastReceivedOffset[key] == offset) { // no more
                ...
                setImmediate(self.thunk('_emitMessage', key, offset + 1));
            }
        });
    });
};
...
MessageConsumer.prototype.rebalance = function (cb /* (err) */) {
    ...
    async.parallel([
        ...
        var peers = vals[0],
            partitions = vals[1],
            consumersPerTopicMap = getConsumersPerTopicMapFromPeers(topics, peers),
            partitionsPerTopicMap = getPartitionsPerTopicMap(partitions, topics),
            oldPayloads = self.consumer.payloads,
            newPayloads = self._makeNewPayloads(topics, consumersPerTopicMap, partitionsPerTopicMap);

        var give = [], take = [], intersection = [];
        diffPayloads(oldPayloads, newPayloads, give, intersection, take);
        newPayloads = take.concat(intersection); // de-dupe
        if (self.options.likeScala) {
            give = oldPayloads;
            take = newPayloads;
        }
        ...
        function potluck (cb) { // attempt to give and take partitions
            async.series([
                self.thunk('onBeforeFetch'), // like self.onBeforeFetch.bind(self)
                self.thunk('_givePartitions', give, intersection), // parallel?
                self.thunk('_takePartitions', take), // commit ownership
                self.thunk('_getCommittedPayloadOffsets', newPayloads), // for partitions previously assigned to a consumer
                self.thunk('_getVirginPayloadOffsets', newPayloads), // for partitions which have never been consumed
                self.thunk('_ensureFetching'),
            ].map(self.rebalanceLock.cancels), cb);
        }
    });
};

// helper so we can copy the scala algorithm verbatim
function getConsumersPerTopicMapFromPeers (topics, peers) {
    var consumersPerTopicMap = {};
...
function getPartitionsPerTopicMap(partitions, topics) {
...
MessageConsumer.prototype._makeNewPayloads = function (topics, consumersPerTopicMap, partitionsPerTopicMap) {
    var self = this;
...
function diffPayloads(oldPayloads, newPayloads, give, intersection, take) {
    var gift = {};
...
MessageConsumer.prototype._givePartitions = function (give, keep, cb) {
    var self = this;
    debug('giving ' + give.length + ' partition(s)');
    this.consumer.payloads = keep;
    give.forEach(function (payload) {
        var key = canonicalPayloadString(payload);
        if (self.lastReceivedOffset.hasOwnProperty(key)) { // we have prefetched messages
            var messageByOffset = self.messageByOffset[key];
            var last = self.lastReceivedOffset[key]; // last received (and discarded)
            var cur = self.lastReceivedOffset[key] = self.lastCommittedOffset[key] + 1;
            console.assert(cur <= last);
            var first = cur + 1; // first discarded
            for (var i = first; i <= last; ++i) {
                delete messageByOffset[i]; // dropped prefetch
            }
        }
    });
    if (!give.length) {
...
MessageConsumer.prototype._takePartitions = function (take, cb) {
    var self = this;
...
MessageConsumer.prototype._getCommittedPayloadOffsets = function (newPayloads, cb) {
    var self = this;
    if (debug.enabled) {
...
MessageConsumer.prototype._getVirginPayloadOffsets = function (newPayloads, cb) {
    var self = this;
...
MessageConsumer.prototype._ensureFetching = function (cb) {
    var self = this;
    ...
    Object.keys(signum).forEach(function (signal) {
        process.on(signal, function (arg) {
            ...
            if (signal === 'uncaughtException') {
                signal = arg;
                console.log(signal, 'arg.stack', arg.stack);
            }
